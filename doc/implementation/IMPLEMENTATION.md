...
**Описание:**
Реализовать все TypeScript-типы сущностей по DATABASE.md: `Project`, `ProjectStatus` (`created | active | deploying | error | stopped`), `Deployment`, `DeployStatus` (`queued | building | deploying | success | failed | cancelled`), `TriggeredBy`, `EnvVariable`, `Domain`, `SslStatus`, `Settings`, `SystemMetrics`, `ApiResponse<T>`, `ApiError`. Константы: `FRAMEWORKS` (таблица детекции из PRD FR-201), `PORT_RANGE_START` (4322), `PORT_RANGE_END` (4400), `DEPLOY_STEPS`.
...
- При нехватке портов (все 4322–4400 заняты) → 409
...
**Описание:**
Консоль деплоя из UI-UX.md раздел 4.7: терминальный эмулятор (чёрный фон `#0a0a0a`, моноширинный шрифт JetBrains Mono 12px, зелёный текст для success, красный для error). `useSSE(url)` hook: подключение к SSE endpoint, парсинг событий, автореконнект при обрыве. `DeployProgress`: визуальный прогресс 8 шагов pipeline (fetch → checkout → install → build → sync → env → restart → healthcheck) — каждый шаг показывает статус (⏳ pending, 🔄 running, ✅ done, ❌ failed). `DeployLog`: ScrollArea с автоскроллом вниз, каждая строка — timestamp + message.
...
- [x] install.sh скрипт протестирован на Ubuntu 22.04
- [x] .env.example с описанием всех переменных
- [x] systemd unit для FrostDeploy

### Production Testing

- [x] 3 тестовых сессии проведены, 18 багов найдено и исправлено
- [x] Сервер запущен на 203.31.40.195:9000 (Node.js 22 + Hono 4 + Drizzle ORM)
- [x] SQLite БД: /var/lib/frostdeploy/data.db
- [x] Caddy v2.11.2 с Admin API на localhost:2019 (caddyFetch wrapper с Origin header)
- [x] Domain flow end-to-end: obrazz-ai.ru с Let's Encrypt SSL
- [x] Порты: 4322–4400 (4321 зарезервирован за La Villa Pine)
- [x] Env vars деплоятся через EnvironmentFile в systemd units
- [x] Backup: ежедневный авто-бэкап, макс. 7 копий, SQLite Online Backup API
...
- `deleteProject` не очищает директории на файловой системе (известный gap)
- Shared-тип `Project` не включает массив `domains` (требуется cast в UI)
- Нет webhook auto-deploy (только ручной деплой)
