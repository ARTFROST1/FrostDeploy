...
**Описание:**
`error-handler.ts`: глобальный `onError`-обработчик, возвращающий `{ error: string, statusCode: number }` JSON. В dev-режиме — stack trace, в production — только сообщение. `logger.ts`: логирование каждого запроса в формате `METHOD /path STATUS DURATIONms`. CORS: разрешить `localhost:5173` (Vite dev server) в dev-режиме.

**Acceptance Criteria:**
- Намеренный `throw new Error('test')` в маршруте → JSON с `{ error: 'Internal Server Error' }`
...
**Описание:**
Оптимизация FR-304: если `project.current_sha === targetSha` и `force !== true`, пропустить сборку. Вернуть SSE-событие с сообщением «Already deployed — skipping build». Если `force: true` — выполнить полный pipeline.
...
**Описание:**
`addRoute(domain, port, isStatic, ...)`: POST к Caddy Admin API для добавления маршрута. `removeRoute(domain)`: DELETE маршрута. `reloadConfig()`: вызов `caddy reload` для применения изменений Caddyfile. Validate-before-apply (NFR-401): при ошибке от Admin API — откат, логирование, возврат ошибки пользователю. Сохранение конфигурации в файл как backup.

**Acceptance Criteria:**
- `addRoute('app.example.com', 4321)` → маршрут появляется в Caddy config
...
**Описание:**
Функция `checkSslStatus(domain)`: запрос к Caddy Admin API для получения статуса сертификата (`GET /config/apps/tls/certificates/automate`). Маппинг: если домен в списке и сертификат валиден → `active`; ожидает выпуска → `provisioning`; ошибка → `error`. Обновление поля `ssl_status` в таблице `domains`.
...
**Описание:**
Функции: `validateConfig()` — запрос к Caddy Admin API `/load` с проверкой текущей конфигурации; `reloadCaddy()` — `caddy reload --config /etc/caddy/Caddyfile`; `getCaddyStatus()` — проверка что Caddy-процесс работает (`systemctl is-active caddy`). Используется на этапе интеграции для проверки здоровья прокси после изменений.
...
- `ui/src/components/ui/toast.tsx` (создать — из shadcn/ui)
- `ui/src/components/ui/toaster.tsx` (создать — из shadcn/ui)

**Описание:**
Реализовать страницу авторизации по wireframe из UI-UX.md раздел 4.1: полноэкранный layout, центрированная карточка с логотипом «❄ FrostDeploy», поле пароля (Input type=password), кнопка «Войти» (Button primary). При ошибке — toast с «Неверный пароль». При rate-limit — toast с «Слишком много попыток. Попробуйте через N секунд». При успехе — redirect на `/`. Подсказка внизу: «Забыли пароль? Сбросьте через CLI: frostdeploy reset-password».
...
**Описание:**
Компонент-обёртка для защищённых маршрутов. При монтировании проверяет `GET /api/auth/check`. Если 401 → redirect на `/login`. Если успех → рендерит children. Во время проверки — показывает skeleton / loading. Используется в AppLayout как wrapper.
...
- `ui/src/components/ui/progress.tsx` (создать — из shadcn/ui)

**Описание:**
Мастер первоначальной настройки из UI-UX.md раздел 4.2: 3 шага (Пароль → GitHub PAT → Домен). Шаг 1: два поля пароля (password + confirm, ≥ 8 символов). Шаг 2: поле PAT, при вводе — валидация через GitHub API (GET /user). Шаг 3: поле домена (FQDN). Прогресс-бар с шагами. Кнопки «Назад» / «Далее» / «Завершить». POST `/api/setup` при финализации. После успеха → redirect на `/login`.
...
- «Завершить» → POST /api/setup → redirect /login
...
**Описание:**
Адаптивность из UI-UX.md раздел 7: на desktop (≥ 1024px) — sidebar развёрнут (240px); на планшетах (768–1023px) — sidebar свёрнут (56px, только иконки); на мобильных (< 768px) — sidebar скрыт, доступ через hamburger menu (Sheet/Drawer). Контент-область: padding 24px (desktop), 16px (mobile). Кнопка collapse/expand sidebar.
...
**Файлы:**
- `ui/src/pages/project-new.tsx` (создать)
...
**Файлы:**
- `ui/src/pages/settings.tsx` (создать)
...
**Описание:**
Интегрировать proxy-service в lifecycle проекта: при `createProject()` с доменом → автоматический вызов `addRoute()` для Caddy + запуск DNS-верификации. При `deleteProject()` → `removeRoute()` для Caddy. При `updateProject()` с изменением домена → remove старого + add нового. Обработка ошибок: если Caddy недоступен — проект создаётся, но отмечается warning.
...
**Описание:**
При первом запуске (нет БД или `setup_completed !== 'true'` в settings) — все маршруты редиректят на `/setup`. После завершения setup wizard → `setup_completed = true` → обычная работа. Server-side: middleware проверяет `isSetupCompleted()` — если false, все маршруты кроме `/api/setup` возвращают `{ setupRequired: true }`. Client-side: AuthGuard проверяет ответ и редиректит на `/setup`.
...
**Описание:**
Систематический проход по всем error-сценариям: API-ответы с ошибками (400, 401, 403, 404, 409, 429, 500) → понятные сообщения для пользователя. Retry-logic для transient ошибок (network, 500). Empty states: нет проектов → приглашение создать первый; нет деплоев → инструкция; нет commits → проверить PAT. UI: компонент `Alert` для предупреждений на уровне страницы. Toast для операций (success/error). Loading states с Skeleton для всех данных.
...
- [ ] Setup wizard работает на чистом VDS
- [ ] Login/logout, сессии, rate-limiting
- [ ] CRUD проектов (create, read, update, delete)
- [ ] Framework detection (все 7+ из таблицы PRD)
- [ ] Deploy полный pipeline: git → build → rsync → restart → health check
- [ ] SSE-стриминг логов деплоя
- [ ] Deploy history
- [ ] Rollback к предыдущему коммиту
- [ ] Env-переменные (CRUD, шифрование, маскировка)
- [ ] Caddy: домены, авто-SSL, DNS-верификация
- [ ] Системные метрики (CPU, RAM, Disk)
- [ ] Journalctl-логи через UI
- [ ] Per-project deploy mutex (no parallel deploys)

### Безопасность

- [ ] Zod-валидация на всех API-маршрутах
- [ ] HMAC-SHA256 сессии с TTL 24ч
- [ ] Rate-limiting (auth: 5/min, API: 100/min)
- [ ] AES-256-GCM шифрование секретов в БД
- [ ] npm ci --ignore-scripts при сборке
- [ ] cgroups-изоляция проектов (CPUQuota, MemoryMax)
- [ ] Security headers (X-Content-Type-Options, X-Frame-Options, CSP)
- [ ] CSRF-защита (SameSite cookie + Origin check)

### Производительность

- [ ] API latency (median) < 100ms
- [ ] Dashboard TTI < 2 секунды
- [ ] Deploy Astro SSR < 60 секунд
- [ ] Idle RSS < 100MB
- [ ] SQLite WAL-режим с оптимальными PRAGMA

### Надёжность

- [ ] Validate-before-apply для Caddy конфигов
- [ ] Ошибка сборки → runtime не затронут
- [ ] Orphaned deploy locks очищаются при старте
- [ ] FrostDeploy systemd unit: Restart=on-failure
- [ ] BД backup API работает
...
- [ ] E2E тест: add → deploy → verify passing
