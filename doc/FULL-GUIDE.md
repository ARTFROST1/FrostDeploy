...
Введите пароль для входа в дашборд. Пароль хранится как bcrypt-хеш.
...
- Назначит свободный порт из диапазона 4321–4399
...
```
fetch → checkout → install (npm ci --include=dev) → build → rsync → restart → healthcheck
```
...
На странице проекта укажите домен → настройте DNS A-запись → FrostDeploy настроит Caddy reverse proxy и получит SSL-сертификат.
...
Каждый деплой проходит через 7 шагов:
...
6. **restart** — `npm ci --omit=dev` в runtimeDir + `systemctl restart <unit>`
7. **healthcheck** — HTTP-запрос к localhost:port, ожидание ответа
...
- Порты назначаются из диапазона **4321–4399**
...
> На основе продакшен-тестирования на Debian 12 VDS (2 vCPU, 3.8 GB RAM). Все 15 обнаруженных багов исправлены. Подробности в [doc/PRODUCTION-TEST-PROGRESS.md](doc/PRODUCTION-TEST-PROGRESS.md).
