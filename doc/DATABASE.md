...
| `output_dir` | TEXT | Да | `'dist'` | Директория с артефактами сборки |
...
| `session_secret` | Да | Секрет для HMAC-подписи cookie |
...
export function seedSettings(db: Database, config: SetupConfig) {
  ...
  insert({ key: 'session_secret', value: encrypt(config.sessionSecret), isEncrypted: true });
}
```
...
| Auth Tag | 16 байт |
| Формат хранения | `base64(iv + ciphertext + authTag)` |
...
```bash
# /etc/frostdeploy/env
FROSTDEPLOY_ENCRYPTION_KEY=<64-символьная hex-строка (32 байта)>
```
...
function getEncryptionKey(): Buffer {
  const hex = process.env.FROSTDEPLOY_ENCRYPTION_KEY;
  if (!hex || hex.length !== 64) {
    throw new Error('FROSTDEPLOY_ENCRYPTION_KEY must be 64 hex characters (32 bytes)');
  }
...
export function encrypt(plaintext: string): string {
  ...
  // Формат: iv (12) + ciphertext (N) + authTag (16) → base64
...
4. Обновить `FROSTDEPLOY_ENCRYPTION_KEY`
...
| FR-601: HMAC-cookie | `settings.session_secret` (зашифрован) |
