...
| `port` | INTEGER | Нет | — | Порт приложения (4322–4400), уникальный |
...
| `port_range_start` | Нет | Начало диапазона портов (по умолч. `4322`) |
| `port_range_end` | Нет | Конец диапазона портов (по умолч. `4400`) |
...
export function seedSettings(db: Database, config: SetupConfig) {
  ...
  insert({ key: 'port_range_start', value: '4322', isEncrypted: false });
  insert({ key: 'port_range_end', value: '4400', isEncrypted: false });
...
### Автоматический бэкап (BackupService)

Встроенный `BackupService` выполняет ежедневный автоматический бэкап через SQLite Online Backup API. Максимум хранится **7 последних** копий — при превышении самая старая удаляется.

| Параметр | Значение |
|---|---|
| Интервал | Ежедневно |
| Максимум копий | 7 |
| Метод | SQLite Online Backup API (`database.backup()`) |
| Директория | `/var/lib/frostdeploy/backups/` |
| Формат имени | `data-YYYYMMDD-HHMMSS.db` |
...
| 1.1 | 2026-04-02 | Статус Completed. Бэкап-секция обновлена под фактический BackupService (daily, 7 max, SQLite Backup API) |
